...
def create_material(data: RawMaterialCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def update_material(material_id: int, data: RawMaterialUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    m = db.query(RawMaterial).filter(RawMaterial.id == material_id).first()
    if not m:
        raise HTTPException(status_code=404, detail="Hammadde bulunamadı")
...
def adjust_stock(material_id: int, data: StockAdjust, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    db.commit()
    db.refresh(m)
    log_activity(db, current_user.id, "STOCK_ADJUST", "RawMaterial", material_id, f"Miktar: {data.quantity}")
    return enrich(m)
