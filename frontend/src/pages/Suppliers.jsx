...
export default function Suppliers() {
  ...
  const load = () => {
    api.get('/suppliers', { params: { skip: (page - 1) * limit, limit, search } })
  ...
  const handleSave = async () => {
    try {
      ...
      else await api.put(`/suppliers/${selected.id}`, form)
  ...
  const handleDelete = async id => {
    ...
    try { await api.delete(`/suppliers/${id}`); toast.success('Silindi'); load() }
  ...
  const columns = [
    ...
    { key: 'address', label: 'Adres', render: r => r.address || '-' },
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={() => downloadExcel('suppliers', 'tedarikciler.xlsx').catch(() => toast.error('Hata'))}
      ...
      {modal && (
        <Modal title={modal === 'create' ? 'Yeni Tedarikçi' : 'Tedarikçi Düzenle'} onClose={() => setModal(null)}>
          <div className="space-y-3">
            {[['name','Ad'],['phone','Telefon'],['email','Email'],['address','Adres']].map(([k,l]) => (
              <div key={k}>
