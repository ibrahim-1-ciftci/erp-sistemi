      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <Link to="/" className="flex items-center gap-2.5 flex-shrink-0">
            ...
            <span className={`font-black text-xl transition-colors hidden sm:block ${transparent ? 'text-white' : 'text-gray-900'}`}>
          ...
          <div className="hidden md:flex items-center gap-6 flex-shrink-0">
            {links.map(l => (
              <Link key={l.to} to={l.to}
                className={`text-sm font-medium transition-colors whitespace-nowrap ${location.pathname === l.to ? activColor : `${textColor} hover:text-blue-500`}`}>
                {l.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2 md:hidden">
            <button onClick={toggleLang}
              className={`text-xs font-bold px-2.5 py-1.5 rounded-full border transition-colors ${transparent ? 'border-white/20 text-white' : 'border-gray-200 text-gray-600'}`}>
              {i18n.language === 'tr' ? 'EN' : 'TR'}
            </button>
            <button className={`p-1 ${textColor}`} onClick={() => setOpen(!open)}>
